...
public class CardValidation {
    public static void validateCard(Card card) {
        if(card.getType()!=null&&card.getValue()!=null&&card.getValue()>0){
            throw new CardException("Bir kartın hem type değeri hem de value değeri olamaz. " +
                    "Value değeri varsa type " + "değer null olmalı.Type değeri varsa value değeri null" +
                    " olmalıdır.", HttpStatus.BAD_REQUEST);
        }
